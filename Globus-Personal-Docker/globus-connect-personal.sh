...
#cd /home/gridftp
cd /home/cloud/Globus-Personal-Docker
...
if [ -d "$gcpconfigdir" ]; then
    ...
    if [ -d "/home/cloud/Globus-Personal-Docker/globus_config/$gcpconfigdir" ]; then 
        # Copy existing config and then start GCP endpoint
        #cp -p -r /home/gridftp/globus_config/.glob* ~
	cp -p -r /home/cloud/Globus-Personal-Docker/globus_config/.glob* ~
        "./$gcpdir/globusconnectpersonal" -start
    else
