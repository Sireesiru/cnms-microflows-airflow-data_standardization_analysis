...
def convert_to_h5(file_path, metadata=None):
    ...
    try:
        ...
        # --- UNTOUCHED MRC PIPELINE ---
        elif ext == '.mrc' and hasattr(reader, 'mrc') and reader.mrc is not None:
            hdr = reader.mrc.header
            extracted_header_meta['MRC_Header_Summary'] = {
                "nx": int(hdr.nx), "ny": int(hdr.ny), "nz": int(hdr.nz),
                "mode": int(hdr.mode), "mx": int(hdr.mx), "my": int(hdr.my), "mz": int(hdr.mz)
            }
...
def execute_automated_pipeline():
    ...
    print("\n" + "="*60)
    print(" PRODUCTION PIPELINE EXECELINE COMPLETE")
    print("="*60)
